...
func TestReadFormat19GroundSupersonicValid(t *testing.T) {
	...
	msg, err := ReadFormat19GroundSpeedSupersonic(buildValidFormat19GroundSupersonicMessage())
	if err != nil {
		t.Fatal(err)
	}
	...
	if msg.GetSubtype() != fields.SubtypeGroundSpeedSupersonic {
		t.Errorf("Expected Subtype \"%v\", got \"%v\"",
			fields.SubtypeGroundSpeedSupersonic.ToString(),
...
func TestReadFormat19GroundSupersonicTooShort(t *testing.T) {

	// Get too short data
	data := buildValidFormat19GroundSupersonicMessage()[:6]

	_, err := ReadFormat19GroundSpeedSupersonic(data)
	if err == nil {
		t.Error(err)
	}
}

func TestReadFormat19GroundSupersonicBadCode(t *testing.T) {
...
func TestReadFormat19GroundSupersonicBadSubType(t *testing.T) {
...
func buildValidFormat19GroundSupersonicMessage() []byte {
	data := make([]byte, 7)

	// 1001 1010: code 19 (10011) + subtype 2 (010)
	data[0] = 0x9A

	// 1101 0011: Intent Change Flag (1) + IFR capability Flag (1) + NUC < 3 m/s (010) + Direction EW East (0) +
	// Velocity EW 4084kt -> 1022 (11 [1111 1110])
	data[1] = 0xD3

	// 1111 1110: Velocity 340kt -> 341 ([11] 1111 1110)
	data[2] = 0xFE

	// 0010 1010: Direction NS North (0) + Velocity NS 0kt -> 1 (000 0000 [001])
	data[3] = 0x00

	// 0011 1010:  Velocity NS 0kt -> 1 ([000 0000] 001) + Source Vertical Baro (1) + Sign Vertical Down (1) +
