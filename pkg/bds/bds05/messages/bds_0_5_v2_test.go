...
func isBDS05V2Valid(t *testing.T, msg MessageBDS05V2) {
	...
	if msg.GetNavigationIntegrityCodeSupplementB() != fields.NICBOne {
		t.Errorf("Expected Time \"%v\", got \"%v\"",
			fields.NICBOne.ToString(),
			msg.GetNavigationIntegrityCodeSupplementB().ToString())
	}
...
func buildValidBDS05V2Message() []byte {
	data := make([]byte, 7)
	...
	// 0000 0101: code empty (00000) + Surveillance Status: Temporary alert (10) + NIC Supplement B (1)
	data[0] = 0x05
