...
type MeteorologicalHazardReportV0 struct {
...
func ReadMeteorologicalHazardReportV0(data []byte) (*MeteorologicalHazardReportV0, error) {
	...
	turbulenceStatus, turbulence := fields.ReadTurbulenceV0(data)
	windShearStatus, windShear := fields.ReadWindShearV0(data)
	microBurstStatus, microBurst := fields.ReadMicroBurstV0(data)
	icingStatus, icing := fields.ReadIcingV0(data)
	wakeVortexStatus, wakeVortex := fields.ReadWakeVortexV0(data)
	staticAirTemperatureStatus, staticAirTemperature := fields.ReadStaticAirTemperatureV0(data)
	averageStaticPressureStatus, averageStaticPressure := fields.ReadAverageStaticPressure(data)
	radioHeightStatus, radioHeight := fields.ReadRadioHeightV0(data)

	return &MeteorologicalHazardReportV0{
