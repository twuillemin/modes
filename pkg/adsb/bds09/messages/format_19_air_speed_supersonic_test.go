...
func TestReadFormat19AirspeedSupersonicValid(t *testing.T) {

	msg, err := ReadFormat19AirSpeedSupersonic(buildValidFormat19AirspeedSupersonicMessage())
	if err != nil {
		t.Fatal(err)
	}
	...
	if msg.GetSubtype() != fields.SubtypeAirspeedSupersonic {
		t.Errorf("Expected Subtype \"%v\", got \"%v\"",
			fields.SubtypeAirspeedSupersonic.ToString(),
	...
	if msg.AirspeedSupersonic.GetAirspeed() != 4084 {
		t.Errorf("Expected Airspeed to be 4084, got \"%v\"",
			msg.AirspeedSupersonic.GetAirspeed())
	}
...
func TestReadFormat19AirspeedSupersonicTooShort(t *testing.T) {

	// Get too short data
	data := buildValidFormat19AirspeedSupersonicMessage()[:6]

	_, err := ReadFormat19AirSpeedSupersonic(data)
	if err == nil {
		t.Error(err)
	}
}

func TestReadFormat19AirspeedSupersonicBadCode(t *testing.T) {

	// Change code to 2
	data := buildValidFormat19AirspeedSupersonicMessage()
...
func TestReadFormat19AirspeedSupersonicBadSubType(t *testing.T) {
...
func buildValidFormat19AirspeedSupersonicMessage() []byte {
	data := make([]byte, 7)

	// 1001 1100: code 19 (10011) + subtype 4 (100)
	data[0] = 0x9C
	...
	// 1111 1111: AirSpeed Type TAs (1) + Velocity 4084 kt => 1022 (111 1111 [110])
	data[3] = 0xFF

	// 1101 1010:  Velocity 4084 kt => 1022 ([111 1111] 110) + Source Vertical Baro (1) + Sign Vertical Down (1) +
